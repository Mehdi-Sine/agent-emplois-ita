...
class ArvalisConnector(BaseConnector):
    def discover_offer_urls(self, client: Client) -> list[str]:
        ...
        return find_offer_links(
            tree,
            str(response.url),
            ["a[href*='/offres-d-emploi-de-stages/']", "article a[href]", ".views-row a[href]"],
        )
    ...
    def parse_offer(self, client: Client, url: str) -> dict[str, Any] | None:
        ...
        description = description_from_selectors(
            tree,
            [".field--name-body", "main article", ".node__content"],
        )
        meta_blob = " ".join(
            filter(
                None,
                [
                    title,
                    node_text(tree, [".field--name-field-localisation", ".field--name-field-lieu", ".job-location"]),
                    description,
                ],
            )
        )
        return {
            ...
            "location_text": node_text(tree, [".field--name-field-localisation", ".field--name-field-lieu", ".job-location"]),
