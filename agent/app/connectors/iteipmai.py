...
class IteipmaiConnector(BaseConnector):
    def discover_offer_urls(self, client: Client) -> list[str]:
        ...
        return find_offer_links(
            tree,
            str(response.url),
            ["a[href*='/actualite/']", "article a[href]", ".news-item a[href]"],
        )
    ...
    def parse_offer(self, client: Client, url: str) -> dict[str, Any] | None:
        ...
        description = description_from_selectors(
            tree,
            [".entry-content", ".post-content", "main article", ".news-content"],
        )
        meta_blob = " ".join(
            filter(
                None,
                [
                    title,
                    node_text(tree, [".location", ".lieu", ".entry-meta"]),
                    description,
                ],
            )
        )
        return {
            ...
            "posted_at": parse_date_guess(node_text(tree, ["time", ".entry-date", ".date"])),
        }
