...
class IfipConnector(BaseConnector):
    def discover_offer_urls(self, client: Client) -> list[str]:
        ...
        return find_offer_links(
            tree,
            str(response.url),
            ["a[href*='/recrutement/']", "main article a[href]", "main a[href]"],
        )
    ...
    def parse_offer(self, client: Client, url: str) -> dict[str, Any] | None:
        ...
        response.raise_for_status()
        tree = html_tree(response.text)
        title = node_text(tree, ["h1", ".entry-title", ".page-title"])
        description = description_from_selectors(
            tree,
            [".entry-content", ".post-content", "main article", ".field--name-body"],
        )
        meta_blob = " ".join(
            filter(
                None,
        ...
        return {
            ...
            "location_text": node_text(tree, [".location", ".lieu"]),
            "contract_type": classify_contract_type(meta_blob),
            "offer_type": classify_offer_type(meta_blob),
            "posted_at": parse_date_guess(node_text(tree, ["time", ".published", ".date"])),
        }
