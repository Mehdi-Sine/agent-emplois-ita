...
class ItaviConnector(BaseConnector):
    ...
    def _discover_company_listing_url(self, tree, base_url: str) -> str:
        iframe = tree.css_first(
            "iframe[src*='careers.flatchr.io/company/'], iframe[src*='careers.flatchr.io/fr/company/']"
        )
        if iframe:
            src = iframe.attributes.get("src")
            slug = self._extract_company_slug(absolute_url(base_url, src))
            if slug:
                return f"https://careers.flatchr.io/fr/company/{slug}/"

        for node in tree.css(
            "a[href*='careers.flatchr.io/company/'], a[href*='careers.flatchr.io/fr/company/']"
        ):
            href = node.attributes.get("href")
            slug = self._extract_company_slug(absolute_url(base_url, href))
            if slug:
                return f"https://careers.flatchr.io/fr/company/{slug}/"
    ...
    def _clean_line(self, value: str | None) -> str:
        ...
        # si la ligne n'est que de la ponctuation, on la vide
    ...
    def _ensure_country(self, location_text: str | None, country: str) -> str | None:
        if not location_text:
            return None

        location_text = self._clean_line(location_text)
        if not location_text:
            return None
        ...
        return f"{location_text} {country}"
    ...
    def _infer_offer_type(
        ...
        # ne pas classer "thèse" juste parce que la description mentionne doctorat
    ...
    def _is_city_candidate(self, value: str | None) -> bool:
        ...
        # rejette les lignes faites uniquement de ponctuation
    ...
    def _extract_city_from_header(
        ...
        for idx, line in enumerate(header_lines):
            ...
            # Cas 1 : ville et CP sur la même ligne, ex: "Rennes, 35000"
            before_zip = clean[: zip_match.start()].strip(" ,;:-–|")
            if self._is_city_candidate(before_zip):
                return normalize_spaces(before_zip)

            # Cas 2 : Flatchr a éclaté le header en plusieurs lignes :
            # "Rennes" / "," / "35000"
